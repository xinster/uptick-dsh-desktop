...
- 💰 **余额/用量浮窗**：左侧栏 Settings 上方常驻浮窗——实时余额（充值/赠送拆分）、**高峰/空闲时段**徽章（官方峰谷定价：UTC 01–04 / 06–10 为高峰，空闲半价）、**今日 token 用量**（本地统计 DSH 会话日志，输入/输出/调用次数，需安装 zstd（`brew install zstd`）），每分钟刷新；颜色自动跟随主应用浅色/深色主题；可鼠标拖动（位置自动记住）；点击展开明细
